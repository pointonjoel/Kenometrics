...
Note that there is not currently support for running an ADF or chow test after you have declared a dataset time series. Consequently, it is reccomended that the original dataset is preserved when the time series function is used. This is a feature which is currently being explored.
...
hseinv_data <- ADF("lpop", hseinv_data, type="type1")
```
